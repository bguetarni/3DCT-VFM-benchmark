    ...
    def get_features_labels(self, path_=None):
        ...
        for id_, p in patients.items():
            ...
            if prog == "Yes" and not(prog_endrt_days is None):
                rfs_T = prog_endrt_days
                rfs_delta = 1
            elif not(endfollow_endrt_days is None):
                rfs_T = endfollow_endrt_days
                rfs_delta = 0
            else:
                rfs_T = None
                rfs_delta = 0
...
class HeadNeckCTAtlas(TCIA):
    ...
    def get_features_labels(self, path_=None):
        ...
        for id_, p in patients.items():
            try:
                if p.clinical["Site of recurrence (Distal/Local/ Locoregional)"] == "Complete response":
                    rfs_T = int(p.clinical["Follow up duration (month)"] * 30.4)
                    rfs_delta = 0
                else:
                    rfs_T = int(p.clinical["Disease-free interval (months)"] * 30.4)
                    rfs_delta = 1
            except KeyError:
                rfs_T = None
                rfs_delta = None
